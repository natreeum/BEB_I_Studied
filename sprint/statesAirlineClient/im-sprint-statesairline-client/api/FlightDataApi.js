...
export async function getFlight(filterBy = {}) {
  ...
  let api = `http://ec2-13-124-90-231.ap-northeast-2.compute.amazonaws.com:81/flight?departure=${filterBy.departure}`;
  if (filterBy.destination) {
    api = api + `&destination=${filterBy.destination}`;
  }
  // const flight = await fetch(api).then((resp) => resp.json());
  // return flight;
