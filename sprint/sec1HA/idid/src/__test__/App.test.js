...
describe("메뉴에는 Gallery, About이라는 두 개의 항목이 존재해야 합니다", () => {
  ...
  it("ul > li 엘리먼트 아래에 <Link> 컴포넌트가 두 개 존재해야 합니다", () => {
    ...
    expect(container.querySelectorAll("ul > li > a")[0].innerHTML).toBe(
      "Gallery"
    );
    expect(container.querySelectorAll("ul > li > a")[1].innerHTML).toBe(
      "About"
    );
  });
  ...
  it("<Link> 컴포넌트는 각각 `/`, `/about` 주소로 이동할 수 있어야 합니다", () => {
    ...
    expect(
      container.querySelector('ul > li > a[href="/about"]').href
    ).toBeDefined();
  });
});
...
describe("페이지 영역이 존재하며, 메뉴 선택에 따라 페이지가 바뀌어야 합니다", () => {
  ...
  it("`#page` 영역에 라우팅에 따른 페이지 분기를 구현해야 합니다", () => {
    render(<App />, { container });
    const routesComponent = resq$("Routes", container);
    expect(routesComponent.children).toBeDefined();
  });
