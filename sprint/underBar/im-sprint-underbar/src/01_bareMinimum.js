...
_.last = function (arr, n) {
  ...
  if (arr.length < n) return rArr;
    ...
    eArr.push(arr[arr.length - 1]);
    return eArr;
  }
};

// _.each는 collection의 각 데이터에 반복적인 작업을 수행합니다.
//  1. collection(배열 혹은 객체)과 함수iteratee(반복되는 작업)를 인자로 전달받아 (iteratee는 함수의 인자로 전달되는 함수이므로 callback 함수)
